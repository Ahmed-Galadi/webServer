...
const int CGIhandler::CGI_TIMEOUT;
...
CGIhandler::CGIhandler() {
    std::cout << "[DEBUG] CGIhandler constructed" << std::endl;
}

CGIhandler::~CGIhandler() {
    std::cout << "[DEBUG] CGIhandler destructed" << std::endl;
}
...
Response* CGIhandler::handler(const Request &req, const LocationConfig* location, const ServerConfig* serverConfig) {
    ...
    std::cout << "[ERROR] Synchronous CGI handler called - this should not happen" << std::endl;
...
bool CGIhandler::startCgiExecution(const Request &req,
    ...
    std::cout << "[DEBUG] Starting async CGI for: " << req.getURI() << std::endl;
    ...
    std::cout << "[DEBUG] CGI script path: " << scriptPath << std::endl;
    
    // Validate script exists and is executable
    if (access(scriptPath.c_str(), X_OK) != 0) {
        std::cerr << "[ERROR] CGI script not found or not executable: " << scriptPath << std::endl;
    ...
    if (socketpair(AF_UNIX, SOCK_STREAM, 0, socketPair) == -1) {
        std::cerr << "[ERROR] socketpair failed" << std::endl;
    ...
    if (!forkAndExecCgi(scriptPath, env, socketPair, pid)) {
        std::cerr << "[ERROR] Failed to fork/exec CGI" << std::endl;
    ...
    std::cout << "[DEBUG] CGI execution started, PID=" << pid << ", FD=" << socketPair[0] << std::endl;
...
bool CGIhandler::forkAndExecCgi(const std::string &scriptPath,
    ...
    if (pid == 0) {
        ...
        if (dup2(socketPair[1], STDIN_FILENO) == -1 ||
            dup2(socketPair[1], STDOUT_FILENO) == -1 ||
            dup2(socketPair[1], STDERR_FILENO) == -1) {
            std::cerr << "[ERROR] dup2 failed in child" << std::endl;
            _exit(127);
        }
        ...
        if (chdir(scriptDir.c_str()) == -1) {
            std::cerr << "[ERROR] chdir failed: " << scriptDir << std::endl;
            _exit(127);
        }
...
void CGIhandler::handleCgiEvent(int fd, uint32_t events, EventManager& eventMgr) {
    ...
    if (it == s_cgiExecutions.end()) {
        std::cerr << "[ERROR] CGI event for unknown FD: " << fd << std::endl;
        return;
    }
    
    CgiExecution* exec = it->second;
    
    std::cout << "[DEBUG] CGI event for FD=" << fd << ", events=" << events << ", state=" << exec->state << std::endl;
    
    // Handle EPOLLERR (true errors)
    if (events & EPOLLERR) {
        std::cout << "[DEBUG] CGI socket error" << std::endl;
    ...
    if (events & EPOLLHUP) {
        std::cout << "[DEBUG] CGI socket hangup (normal termination)" << std::endl;
        if (exec->state == CGI_READING_OUTPUT) {
            ...
            if (s_cgiExecutions.find(fd) == s_cgiExecutions.end()) {
                std::cout << "[DEBUG] CGI already finalized by handleCgiRead" << std::endl;
                return;
            }
        }
...
void CGIhandler::handleCgiWrite(CgiExecution* exec, EventManager& eventMgr) {
    if (exec->requestBody.empty() || exec->bodyBytesWritten >= exec->requestBody.size()) {
        ...
        std::cout << "[DEBUG] Request body write complete, switching to read mode" << std::endl;
    ...
    if (written > 0) {
        ...
        std::cout << "[DEBUG] Wrote " << written << " bytes to CGI (" 
                  << exec->bodyBytesWritten << "/" << exec->requestBody.size() << ")" << std::endl;
        
        // Check if done
        if (exec->bodyBytesWritten >= exec->requestBody.size()) {
            std::cout << "[DEBUG] Request body fully written, switching to read mode" << std::endl;
            shutdown(exec->socketFd, SHUT_WR);
            exec->state = CGI_READING_OUTPUT;
            eventMgr.modifySocket(exec->socketFd, exec, EPOLLIN | EPOLLERR | EPOLLHUP);
        }
        ...
        std::cout << "[DEBUG] CGI write would block or error" << std::endl;
    }
}
...
void CGIhandler::handleCgiRead(CgiExecution* exec, EventManager& eventMgr) {
    ...
    if (bytes > 0) {
        ...
        std::cout << "[DEBUG] Read " << bytes << " bytes from CGI (total: " 
                  << exec->output.size() << ")" << std::endl;
    } else if (bytes == 0) {
        // EOF - CGI finished writing
        std::cout << "[DEBUG] CGI finished writing (EOF)" << std::endl;
        exec->state = CGI_COMPLETE;
        finalizeCgiExecution(exec, eventMgr);
    } else {
        // Error or would block
        std::cout << "[DEBUG] CGI read would block or error" << std::endl;
    }
}
...
void CGIhandler::finalizeCgiExecution(CgiExecution* exec, EventManager& eventMgr) {
    std::cout << "[DEBUG] Finalizing CGI execution" << std::endl;
    ...
    if (result == 0) {
        ...
        if (WIFEXITED(status)) {
            ...
            exec->scriptExitCode = -1;
            std::cout << "[DEBUG] CGI killed by signal" << std::endl;
        }
    } else {
        // waitpid failed
        std::cout << "[DEBUG] waitpid failed" << std::endl;
        exec->scriptExitCode = -1;
    }
    ...
    std::cout << "[DEBUG] CGI execution finished:" << std::endl;
    std::cout << "[DEBUG]   State: " << exec->state << std::endl;
    std::cout << "[DEBUG]   Exit code: " << exec->scriptExitCode << std::endl;
    std::cout << "[DEBUG]   Output size: " << exec->output.size() << " bytes" << std::endl;
    if (exec->output.size() > 0 && exec->output.size() <= 500) {
        std::cout << "[DEBUG]   Output: " << exec->output << std::endl;
    }
    ...
    if (exec->state == CGI_TIMEOUT) {
        ...
        std::cout << "[ERROR] CGI script exited with non-zero code: " << exec->scriptExitCode << std::endl;
        response = new Response();
        response->setStatus(500);
        response->setReasonPhrase("Internal Server Error");
        response->setBody(exec->output.empty() ? 
...
void CGIhandler::cleanupCgiExecution(int fd, EventManager& eventMgr) {
    ...
    std::cout << "[DEBUG] Cleaning up CGI execution, FD=" << fd << std::endl;
...
void CGIhandler::checkCgiTimeouts(EventManager& eventMgr) {
    ...
    while (it != s_cgiExecutions.end()) {
        ...
        if (now - exec->startTime > CGI_TIMEOUT) {
            std::cout << "[DEBUG] CGI timeout detected for FD=" << it->first << std::endl;
            exec->state = static_cast<CgiState>(CGI_TIMEOUT);
...
Response* CGIhandler::parseCgiOutput(const std::string &output, int exitCode) {
    std::cout << "[DEBUG] Parsing CGI output (" << output.size() << " bytes), exit=" << exitCode << std::endl;
