...
Response* GEThandler::handler(const Request& req, const LocationConfig* location, const ServerConfig* /* serverConfig */) {
    ...
    try {
        ...
        std::cout << "[DEBUG] GET handler - Resolved path: " << path << std::endl;

        // Check if path exists (file or directory)
        if (!fileExists(path) && !isDirectory(path)) {
            std::cout << "[DEBUG] Path does not exist: " << path << std::endl;
            delete response;
        ...
        // ============================================
        if (isDirectory(path)) {
            std::cout << "[DEBUG] Path is a directory: " << path << std::endl;

            // Fix 3: redirect /dir -> /dir/
            if (uri[uri.length() - 1] != '/') {
                std::string redirectUri = uri + "/";
                std::cout << "[DEBUG] Redirecting to: " << redirectUri << std::endl;
                delete response;
            ...
            std::cout << "[DEBUG] Index directive: '" << indexFile << "'" << std::endl;
            std::cout << "[DEBUG] Autoindex setting: " << (autoindex ? "on" : "off") << std::endl;
            
            // Check if index directive is configured and not empty
            if (!indexFile.empty()) {
                std::string indexPath = dirPath + indexFile;
                std::cout << "[DEBUG] Looking for index file: " << indexPath << std::endl;
                ...
                if (fileExists(indexPath)) {
                    ...
                    std::cout << "[DEBUG] Index file not found: " << indexPath << std::endl;
                    
                    if (autoindex) {
                        // Case 3: Index not found + autoindex ON → directory listing
                        std::cout << "[DEBUG] Autoindex ON, generating directory listing" << std::endl;
                        delete response;
                        return generateDirectoryListing(uri, dirPath, req);
                    } else {
                        // Case 4: Index not found + autoindex OFF → 403 Forbidden
                        std::cout << "[DEBUG] Autoindex OFF, returning 403" << std::endl;
                        delete response;
                ...
                std::cout << "[DEBUG] No index directive configured" << std::endl;
        ...
        std::cout << "[DEBUG] Serving file: " << path << std::endl;

        // Security check: ensure path is not a directory
        if (isDirectory(path)) {
            std::cout << "[ERROR] Attempting to serve directory as file: " << path << std::endl;
            delete response;
            return createErrorResponse(403, "Forbidden");
        }
        ...
        if (mimeType == "application/octet-stream") {
            ...
            std::cout << "[DEBUG] Unknown MIME type, triggering download for: " << filename << std::endl;
        }
        ...
        std::cout << "[DEBUG] Successfully served file, size: " << responseBody.size() << " bytes" << std::endl;
        return response;

    } catch (const std::exception& e) {
        std::cout << "[ERROR] Exception in GET handler: " << e.what() << std::endl;
        delete response;
...
Response* GEThandler::generateDirectoryListing(const std::string& uri, 
                                               ...
                                               const Request& req) {
    std::cout << "[DEBUG] Generating directory listing for: " << dirPath << std::endl;
    ...
    if (!dir) {
        std::cout << "[ERROR] Cannot open directory: " << dirPath << std::endl;
        delete response;
    ...
    std::cout << "[DEBUG] Directory listing generated successfully" << std::endl;
